...
export default async function ChannelPage({ params }: { params: { handle: string } }) {
  ...
  const cleanHandle = decodeURIComponent(params.handle);
  ...
  const isSubscribed = session?.user?.id
    ? !!(await prisma.subscription.findUnique({
        where: { subscriberId_channelId: { subscriberId: session.user.id, channelId: channel.id } },
      }))
    : false;
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-end gap-4 -mt-10 mb-8">
          <div className="relative w-20 h-20 rounded-full border-4 border-gray-950 overflow-hidden flex-shrink-0 bg-gray-700">
            {channel.image
              ...
              : <div className="w-full h-full flex items-center justify-center text-white text-2xl font-bold">{channel.name?.[0]?.toUpperCase() ?? "?"}</div>
            }
          </div>
          ...
          <div className="flex-1 min-w-0 pt-10 sm:pt-0">
            ...
            <p className="text-gray-400 text-sm mt-0.5">
              {channel.channelHandle} · {channel._count.subscribers.toLocaleString()} subscribers · {channel._count.videos.toLocaleString()} videos
            </p>
            {channel.bio && <p className="text-gray-500 text-sm mt-2 line-clamp-2">{channel.bio}</p>}
          </div>

          <div className="flex gap-3 sm:ml-auto flex-shrink-0">
            {isOwner
              ? <a href="/settings/profile" className="px-5 py-2 rounded-full bg-gray-700 hover:bg-gray-600 text-white text-sm font-medium transition">Edit channel</a>
              : <SubscribeButton channelId={channel.id} initialSubscribed={isSubscribed} isLoggedIn={!!session} />
            }
          </div>
        </div>

        <VideoGrid initialVideos={videos as VideoWithUser[]} initialCursor={nextCursor} userId={channel.id} />
      </div>
    </main>
  );
}
