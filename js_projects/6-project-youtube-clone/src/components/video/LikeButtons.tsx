  ...
  async function handleReaction(type: LikeType) {
    ...
    startTransition(() => {
      if (userLike === type) {
        ...
        if (previousLike === "LIKE" && type === "DISLIKE")
          setLikeCount((c) => c - 1);
        if (previousLike === null && type === "LIKE")
          setLikeCount((c) => c + 1);
        if (previousLike === "DISLIKE" && type === "LIKE")
          setLikeCount((c) => c + 1);
        setUserLike(type);
      }
    });
  ...
  return (
    <div className="flex items-center gap-1 rounded-full bg-neutral-800 p-1">
      ...
      >
        <svg
          className="h-4 w-4"
          fill={userLike === "LIKE" ? "currentColor" : "none"}
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
          <path
      ...
      >
        <svg
          className="h-4 w-4"
          fill={userLike === "DISLIKE" ? "currentColor" : "none"}
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
          <path
