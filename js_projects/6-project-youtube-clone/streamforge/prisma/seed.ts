...
async function main() {
  ...
  // Add likes
  await Promise.all([
    prisma.like.create({ data: { type: 'LIKE', userId: users[1].id, videoId: videos[0].id } }),
    prisma.like.create({ data: { type: 'LIKE', userId: users[2].id, videoId: videos[0].id } }),
    prisma.like.create({ data: { type: 'LIKE', userId: users[0].id, videoId: videos[1].id } }),
    prisma.like.create({ data: { type: 'LIKE', userId: users[2].id, videoId: videos[2].id } }),
  ])
  console.log('✅ Created likes')

  // Add subscriptions
  await Promise.all([
    prisma.subscription.create({ data: { followerId: users[1].id, followingId: users[0].id } }),
    prisma.subscription.create({ data: { followerId: users[2].id, followingId: users[0].id } }),
    prisma.subscription.create({ data: { followerId: users[0].id, followingId: users[2].id } }),
  ])
