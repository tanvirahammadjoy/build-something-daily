    ...
    <main className="min-h-screen bg-ink">
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="mt-4 flex items-start justify-between gap-4 flex-wrap">
          <h1 className="text-xl font-semibold text-paper flex-1 min-w-0">{video.title}</h1>
        ...
        <div className="flex items-center justify-between mt-4 flex-wrap gap-3">
          ...
          >
            <div className="w-10 h-10 rounded-full overflow-hidden bg-ink-raised flex-shrink-0">
              {video.user.image ? (
            ...
            <div>
              <p className="text-paper text-sm font-medium">{video.user.name}</p>
        ...
        <div className="mt-4 bg-ink-surface rounded-xl p-4">
          <p className="text-paper-faint text-sm">
            {formatViews(video.views)} · {formatRelativeTime(video.createdAt)}
          </p>
          {video.description && (
            <p className="text-paper-dim text-sm mt-2 whitespace-pre-wrap">{video.description}</p>
          )}
          {video.tags.length > 0 && (
            <div className="flex flex-wrap gap-2 mt-3">
              {video.tags.map((t) => (
                <Link key={t.name} href={`/search?tag=${t.name}`} className="text-flare text-xs hover:text-flare-dim">
                  #{t.name}
                </Link>
              ))}
            </div>
