...
function verifyCloudinaryWebhook(body: string, signature: string, timestamp: string): boolean {
  const secret = process.env.CLOUDINARY_WEBHOOK_SECRET!;
  const expected = createHmac("sha1", secret).update(body + timestamp).digest("hex");
...
export async function POST(req: NextRequest) {
  ...
  if (notification_type !== "eager") return new Response(JSON.stringify({ received: true }));

  const hlsUrl = eager?.[0]?.secure_url ?? null;
  const video = await prisma.video.findFirst({ where: { publicId: public_id } });
