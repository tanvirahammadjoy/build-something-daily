...
export function AccountMenu({ user }: { user: SessionUser }) {
  ...
  return (
    <div className="relative" ref={menuRef}>
      ...
      >
        {user.image ? (
          <Image src={user.image} alt="Your avatar" width={32} height={32} className="object-cover" />
        ) : (
        ...
        <div className="absolute right-0 top-11 w-64 bg-ink-surface border border-ink-border rounded-xl2 shadow-xl overflow-hidden z-50">
          <div className="flex items-center gap-3 px-4 py-3 border-b border-ink-border">
            <div className="w-10 h-10 rounded-full overflow-hidden bg-ink-raised flex-shrink-0 ring-1 ring-ink-border">
              {user.image ? (
                <Image src={user.image} alt="Your avatar" width={40} height={40} className="object-cover" />
              ) : (
                <div className="w-full h-full flex items-center justify-center text-paper text-sm font-medium font-display">
                  {user.name?.[0]?.toUpperCase() ?? "?"}
                </div>
            ...
            <div className="min-w-0">
              <p className="text-paper text-sm font-medium truncate">{user.name}</p>
              {user.channelHandle && <p className="text-paper-faint text-xs truncate">{user.channelHandle}</p>}
            </div>
          </div>
...
function MenuIcon({ name }: { name: string }) {
  ...
  return (
    <svg className="w-4 h-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
      <path strokeLinecap="round" strokeLinejoin="round" d={paths[name] ?? paths.user} />
    </svg>
  );
}
