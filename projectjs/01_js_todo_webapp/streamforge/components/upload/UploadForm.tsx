...
export function UploadForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Upload video
      const { public_id, secure_url, duration } = await uploadToCloudinary(file, sig);
    ...
    <div className="space-y-6">
      ...
      <div
        ...
        className={`border-2 border-dashed rounded-xl2 p-10 text-center cursor-pointer transition ${
          file ? "border-flare bg-flare-tint" : "border-ink-border hover:border-ink-borderStrong"
        }`}
      >
        <input
          ref={fileInputRef}
        ...
        {file ? (
          ...
          <div>
            ...
            <p className="text-paper-dim font-medium font-display">Drop your video here or click to browse</p>
            <p className="text-paper-faint text-sm mt-1">MP4, MOV, AVI · Max 2 GB</p>
          </div>
        )}
      </div>
      ...
      <div className="space-y-4">
        <div>
          <label className="block text-sm text-paper-faint mb-1.5">Title</label>
          <input
            type="text"
            value={form.title}
        ...
        <div>
          <label className="block text-sm text-paper-faint mb-1.5">
            Description <span className="text-paper-faint/70">(optional)</span>
          </label>
          <textarea
        ...
        <div>
          <label className="block text-sm text-paper-faint mb-1.5">
            Tags <span className="text-paper-faint/70">(comma-separated)</span>
          </label>
          <input
            type="text"
            value={form.tags}
        ...
        <div>
          <label className="block text-sm text-paper-faint mb-1.5">
            Custom thumbnail <span className="text-paper-faint/70">(optional)</span>
          </label>
          <input
            type="file"
      ...
      {(state === "uploading" || state === "saving") && (
        <div>
          <div className="flex justify-between text-sm text-paper-faint mb-1.5">
            <span>{state === "uploading" ? "Uploading to Cloudinary..." : "Saving..."}</span>
      ...
      <button
        ...
        disabled={!file || !form.title || state === "uploading" || state === "saving"}
